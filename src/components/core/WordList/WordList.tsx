...
export const WordList: React.FC<WordListProps> = ({ className = "" }) => {
  ...
  const getWordClass = (word: Word) => {
    let baseClass =
      "py-2 px-3 rounded-xl mr-2 mb-2 inline-block transition-all font-bold border-2";
  ...
  if (state.foundWords.length === 0) {
    return (
      <div
        className={`bg-blue-500 dark:bg-blue-600 border-4 border-yellow-400 rounded-xl p-4 shadow-lg ${className}`}
      >
        <h2 className="text-xl font-black mb-2 text-white dark:text-white">
          Found Words
        </h2>
  ...
  return (
    <div
      className={`bg-blue-500 dark:bg-blue-600 border-4 border-yellow-400 rounded-xl p-4 shadow-lg overflow-y-auto ${className}`}
    >
      <div className="flex justify-between items-center mb-3">
        <h2 className="text-xl font-black text-white dark:text-white">
          Found Words
        </h2>
