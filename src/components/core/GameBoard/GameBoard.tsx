...
export const GameBoard: React.FC<GameBoardProps> = ({ onWordSubmit }) => {
  ...
  const handleMouseMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (
      ...
      if (isAdjacent) {
        console.log(
          `Selecting at (${position.row},${position.col}) from (${lastPosition?.row},${lastPosition?.col})`
        );
        selectLetter(position);
      }
    }
  ...
  const getCellClass = (row: number, col: number) => {
    ...
    let baseClass =
      "relative flex items-center justify-center font-black rounded-lg border border-gray-400 transition-all cursor-pointer select-none aspect-square";
